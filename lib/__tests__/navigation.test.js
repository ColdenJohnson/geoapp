...
describe('buildViewPhotoChallengeRoute', () => {
  it('builds the expected route params for a challenge detail screen', () => {
    expect(buildViewPhotoChallengeRoute({
      ...
      optimisticPhotoUrls: ['https://one', 'https://two'],
    })).toEqual({
      ...
      params: {
        ...
        optimistic_photo_urls: JSON.stringify(['https://one', 'https://two']),
      },
    });
  });

  it('omits optimistic photo params when there are no valid urls', () => {
    expect(buildViewPhotoChallengeRoute({
      pinId: 42,
      message: null,
      createdByHandle: undefined,
      optimisticPhotoUrls: ['', null, undefined],
    })).toEqual({
      pathname: '/view_photochallenge',
      params: {
        pinId: '42',
