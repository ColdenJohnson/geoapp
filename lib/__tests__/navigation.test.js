const {
  buildViewPhotoChallengePhotoRoute,
  buildViewPhotoChallengeRoute,
  goBackOrHome,
} = require('../navigation');
...
describe('buildViewPhotoChallengePhotoRoute', () => {
  it('builds the expected nested route params for a challenge photo detail screen', () => {
    expect(buildViewPhotoChallengePhotoRoute({
      pinId: 'pin-123',
      photoId: 'photo-456',
    })).toEqual({
  ...
  it('normalizes challenge photo params to strings', () => {
    expect(buildViewPhotoChallengePhotoRoute({
      pinId: 42,
      photoId: 99,
    })).toEqual({
