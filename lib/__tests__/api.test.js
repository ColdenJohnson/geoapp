...
describe('fetch helpers', () => {
  ...
  it('fetchDuelByPinId coerces missing photos to []', async () => {
    ...
    await expect(fetchDuelByPinId('pin')).resolves.toEqual([]);
  });
...
describe('vote endpoints', () => {
  it('voteDuel posts payload', async () => {
    ...
    await expect(
      voteDuel({ pinId: 'pin', winnerPhotoId: 'w', loserPhotoId: 'l' })
    ).resolves.toEqual({ success: true });
    expect(axios.post).toHaveBeenCalledWith(
      `${BASE_URL}/vote_duel`,
      { pinId: 'pin', winnerPhotoId: 'w', loserPhotoId: 'l' },
      { headers: { Authorization: 'Bearer token-42' } }
    );
  });
  ...
  it('voteGlobalDuel handles errors by returning default payload', async () => {
    ...
    axios.post.mockRejectedValue(new Error('bad'));

    await expect(
      voteGlobalDuel({ winnerPhotoId: 'w', loserPhotoId: 'l' })
