...
newChallenge(location, file_url, message, options = {}) {
  try {
    ...
    const photoLocation = normalizeCoordinate(options?.photoLocation)
      || normalizeCoordinate({ latitude, longitude });
...
export async function fetchGlobalDuel() {
  try {
    ...
    const bucketType = typeof response.data?.bucketType === 'string' ? response.data.bucketType : null;
    const pinPrompt = typeof response.data?.pinPrompt === 'string' ? response.data.pinPrompt : null;
    const pinId = response.data?.pinId ? String(response.data.pinId) : null;
    devLogToken('Received global duel payload', voteToken, expiresAt);
    return { photos, voteToken, expiresAt, remainingVotes, bucketType, pinPrompt, pinId };
  } catch (error) {
...
export async function fetchUserTopPhotos(uid, { limit = 2, metric = 'global' } = {}) {
  try {
    const normalizedMetric = metric === 'local' ? 'local' : 'global';
    const parsedLimit = Number.parseInt(String(limit), 10);
    const normalizedLimit = Number.isFinite(parsedLimit)
      ? Math.min(Math.max(parsedLimit, 1), 10)
      : 2;
    const response = await apiGet(
      `/user_top_photos/${uid}?limit=${normalizedLimit}&metric=${normalizedMetric}`
    );
...
export async function fetchFriendActivity() {
  try {
    const response = await apiGet('/friends/activity');
    const items = Array.isArray(response.data?.items) ? response.data.items : [];
    const suggestions = Array.isArray(response.data?.suggestions) ? response.data.suggestions : [];
    console.log('Fetched friend activity:', { items: items.length, suggestions: suggestions.length });
