...
newChallenge(location, file_url, message, options = {}) {
  try {
    ...
    return {
      ...
      newly_earned_achievements: Array.isArray(response.data?.newly_earned_achievements)
        ? response.data.newly_earned_achievements
        : [],
    };
  } catch (error) {
...
export async function addPhoto(pinId, file_url, options = {}) {
  try {
    ...
    return response.data || { success: true, stats: null, newly_earned_achievements: [] };
  } catch (error) {
    console.error('Error sending addphotochallenge log to server:', error);
    throw error;
  }
...
export async function fetchPhotoReactions(photoId) {
  try {
    const response = await apiGet(`/photos/${photoId}/reactions`);
    return {
      groups: Array.isArray(response.data?.groups) ? response.data.groups : [],
      viewer_emoji: typeof response.data?.viewer_emoji === 'string' ? response.data.viewer_emoji : null,
    };
  } catch (error) {
    console.error('Failed to fetch reactions for photo:', photoId, error);
    return { groups: [], viewer_emoji: null };
  }
}

export async function addPhotoReaction(photoId, emoji) {
  try {
    await apiPost(`/photos/${photoId}/reactions`, { emoji });
    return true;
  } catch (error) {
    console.error('Failed to add reaction for photo:', photoId, error);
    return false;
  }
}

export async function deletePhotoReaction(photoId) {
  try {
    await apiDelete(`/photos/${photoId}/reactions`);
    return true;
  } catch (error) {
    console.error('Failed to delete reaction for photo:', photoId, error);
    return false;
  }
}

function normalizeGlobalDuelPayload(payload = {}) {
  const photos = Array.isArray(payload?.photos) ? payload.photos : [];
  const voteToken = typeof payload?.voteToken === 'string' ? payload.voteToken : null;
  const expiresAt = typeof payload?.expiresAt === 'string' ? payload.expiresAt : null;
  const remainingVotes = Number.isFinite(payload?.remainingVotes) ? payload.remainingVotes : null;
  const photoIds = Array.isArray(payload?.photoIds)
    ? payload.photoIds.map((id) => String(id)).filter(Boolean).slice(0, 2)
    : [];
  const pairKey = typeof payload?.pairKey === 'string' ? payload.pairKey : null;
  const bucketType = typeof payload?.bucketType === 'string' ? payload.bucketType : null;
  const pinId = typeof payload?.pinId === 'string' ? payload.pinId : null;
  return {
    photos,
    voteToken,
    expiresAt,
...
export async function fetchGlobalDuel() {
  try {
    ...
    const { photos, voteToken, expiresAt, remainingVotes, photoIds, pairKey, bucketType, pinId } =
      normalizeGlobalDuelPayload(response.data);
    devLogToken('Received global duel payload', voteToken, expiresAt);
    return { photos, voteToken, expiresAt, remainingVotes, photoIds, pairKey, bucketType, pinId };
  } catch (error) {
...
export async function fetchGlobalDuelPool(count = 8) {
  try {
    ...
    const query = Number.isFinite(parsedCount) && parsedCount > 0
      ? `?count=${parsedCount}`
      : '';
    const response = await apiGet(`/global_duels${query}`);
    const items = Array.isArray(response.data?.items)
      ? response.data.items.map((item) => normalizeGlobalDuelPayload(item))
      : [];
    const remainingVotes = Number.isFinite(response.data?.remainingVotes) ? response.data.remainingVotes : null;
    return { items, remainingVotes };
  } catch (error) {
    console.error('Failed to fetch global duel pool:', error);
    return {
      items: [],
      remainingVotes: null,
    };
...
export async function sendQuestChallenge(pinId, recipientUid) {
  try {
    const response = await apiPost('/quests/challenge', { pinId, recipientUid });
    return response.data || { success: true };
  } catch (error) {
    ...
    return {
      success: false,
      code,
      error: error?.response?.data?.error || 'Failed to send challenge',
    };
  }
}

export async function declineQuestChallenge(challengeId) {
  try {
    const response = await apiPost(`/quests/challenge/${encodeURIComponent(challengeId)}/decline`, {});
    return response.data || { success: true };
  } catch (error) {
    return {
      success: false,
      error: error?.response?.data?.error || 'Failed to decline challenge',
    };
  }
}

/*
Admin-only: fetch all non-geo quests for manual tag review.
*/
export async function fetchAdminQuests() {
  try {
    const response = await apiGet('/admin/quests');
    return Array.isArray(response.data?.items) ? response.data.items : [];
  } catch (error) {
    console.error('Failed to fetch admin quests:', error);
    return [];
  }
}

/*
Admin-only: replace a quest's stored tags.
*/
export async function updateAdminQuestTags(pinId, tags) {
  try {
    const response = await apiPatch(`/admin/quests/${encodeURIComponent(pinId)}/tags`, {
    ...
    console.error('Failed to update admin quest tags:', error);
    return {
      success: false,
      error: error?.response?.data?.error || 'Failed to update quest tags',
    };
  }
}
...
export async function fetchAchievementCatalog() {
  try {
    const response = await apiGet('/achievement_catalog');
    const catalog = Array.isArray(response.data) ? response.data : [];
    console.log('Fetched achievement catalog:', catalog.length);
    return catalog;
  } catch (error) {
...
/* Fetch all photos uploaded by a user, sorted newest first, for their private gallery.
Returns { photos: [...], nextCursor: <ISO string> | null } for cursor-based pagination. */
export async function fetchUserGallery(uid, { cursor, limit = 50 } = {}) {
  try {
    const params = new URLSearchParams({ limit: String(limit) });
    if (cursor) params.set('cursor', cursor);
    const response = await apiGet(`/user_gallery/${uid}?${params.toString()}`);
    return response.data ?? { photos: [], nextCursor: null };
  } catch (error) {
    console.error('Failed to fetch gallery for uid:', uid, error);
...
export async function dismissFriendSuggestion(targetUid) {
  try {
    const response = await apiPost('/friends/suggestion/dismiss', { target_uid: targetUid });
    return response.data;
  } catch (error) {
    console.error('Failed to dismiss friend suggestion:', error);
    return { success: false, error: error?.response?.data?.error || 'Failed to dismiss suggestion' };
  }
}
...
export async function fetchFriendActivity(options = {}) {
  try {
    ...
    const interactionSuggestions = Array.isArray(response.data?.interaction_suggestions) ? response.data.interaction_suggestions : [];
    const pendingChallenges = Array.isArray(response.data?.pending_challenges) ? response.data.pending_challenges : [];
    const nextCursor = response.data?.next_cursor && typeof response.data.next_cursor === 'object'
      ? response.data.next_cursor
      : null;
    console.log('Fetched friend activity:', { items: items.length, suggestions: suggestions.length, interactionSuggestions: interactionSuggestions.length, pendingChallenges: pendingChallenges.length, hasNext: !!nextCursor });
    return { items, suggestions, interactionSuggestions, pendingChallenges, nextCursor };
  } catch (error) {
    console.error('Failed to fetch friend activity:', error);
    return { items: [], suggestions: [], interactionSuggestions: [], nextCursor: null };
  }
}
