...
export async function fetchGlobalDuel() {
  try {
    const headers = await getAuthHeader();
    const response = await axios.get(`${PUBLIC_BASE_URL}/global_duel`, { headers });
    return Array.isArray(response.data?.photos) ? response.data.photos : [];
  } catch (error) {
...
export async function voteGlobalDuel({ winnerPhotoId, loserPhotoId }) {
  try {
    const headers = await getAuthHeader();
    const response = await axios.post(`${PUBLIC_BASE_URL}/vote_duel_global`, {
      winnerPhotoId,
      loserPhotoId
    }, { headers });
    return response.data;
...
export async function deleteMyAccount() {
  try {
    const headers = await getAuthHeader();
    const response = await axios.delete(`${PUBLIC_BASE_URL}/delete_account`, { headers });
