...
newPhotoChallenge(location, file_url) {
  try {
    ...
    const pinId = response.data.pinId; // Assuming your backend returns { pinId: ... }
    if (!pinId) {
      throw new Error('No pinId returned from server');
    }

    await addPhotoChallenge(pinId, file_url);
...
export async function addPhotoChallenge(pinId, file_url) {
  try {
    console.log('calling addPhotoChallenge with pin id:', pinId);
    const response = await axios.post(`${PUBLIC_BASE_URL}/add_challenge`, {
      pinId: pinId,
      file_url: file_url
    });
