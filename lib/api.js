...
export async function fetchFriendActivity(options = {}) {
  try {
    ...
    const nextCursor = response.data?.next_cursor && typeof response.data.next_cursor === 'object'
      ...
      : null;
    console.log('Fetched friend activity:', { items: items.length, suggestions: suggestions.length, pendingChallenges: pendingChallenges.length, hasNext: !!nextCursor });
    return { items, suggestions, pendingChallenges, nextCursor };
  } catch (error) {
