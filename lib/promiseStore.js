...
const uploadSubmitResolvers = new Map();
...
export function setUploadSubmitResolver(resolver, requestId = DEFAULT_UPLOAD_RESOLVER_KEY) {
  uploadSubmitResolvers.set(requestId, resolver);
}

export function resolveUploadSubmit(result, requestId = DEFAULT_UPLOAD_RESOLVER_KEY) {
  const resolver = uploadSubmitResolvers.get(requestId);
  if (resolver) {
    resolver(result);
    uploadSubmitResolvers.delete(requestId);
  }
}
